                            ...
                            double b[n])
{
  ...
  r = a[0] * b[0];
  for(i = 1;i < n;i++)// Como i começa em 1 quando n=1 nao correr a primeira vez
  {
    counter1++; // count the number of loop iterations
    r += a[i] * b[i];
  } 
  return r;
}
...
{ // r = a + b
  ...
  for(i = 0;i < n;i++)
  {
    counter2++; // count the number of loop iterations
    r[i] = a[i] + b[i];
  ...
  switch(atoi(argv[1])) // convert the first (and only) argument to an integer
  {
    case 1:
      ...
      for(n = 1;n <= 20;n++)
      {
        ...
        (void)vector_sum(n,a);
        (void)vector_inner_product(n,a,b);
        (void)vector_addition(n,a,b,r);
        (void)factorial(n); // ignore the return value!
        printf(" %10ld",counter);
        printf(" %16ld",counter1);
        printf(" %13ld",counter2);
        printf(" %10ld",counter3);
        //
    ...
    case 2:
      printf("data for the O(n²) functions\n");
      printf("\n");
      printf(" n matrix_sum matrix_vector_product matrix_addition insertion_sort\n");
      printf("-- ---------- -------------------- --------------- ---------\n");
      for(n = 1;n <= 20;n++)
      {
        ...
        for(i = 0;i < n;i++)
        {
          for(j=0;j<n;j++)
          {  
            A[i][j] = (double)rand() / (double)RAND_MAX; // an uniformly distributed pseudo-random number
            B[i][j] = (double)rand() / (double)RAND_MAX; // an uniformly distributed pseudo-random number
            R[i][j] = (double)rand() / (double)RAND_MAX; // an uniformly distributed pseudo-random number
          }
          b[i] = (double)rand() / (double)RAND_MAX; // b[i] =n-i Temos um array de ordem decrescente  e ele vai ordená-lo
          r[i] = (double)rand() / (double)RAND_MAX;
        ...
        (void)matrix_sum(n,A);
        (void)matrix_vector_product(n,A,b,r);
        (void)matrix_addition(n,A,B,R);
        (void)insertion_sort(n,b); // ignore the return value!
        printf(" %10ld",counter);
        printf(" %16ld",counter1);
        printf(" %13ld",counter2);
        printf(" %10ld",counter3);
